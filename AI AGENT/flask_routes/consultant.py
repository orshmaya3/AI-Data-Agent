...
def _build_survey_opening_prompt(plan) -> str:
    ...
    return (
        ...
        "1. Open with a warm but direct greeting referencing their specific goal and how many days have passed.\n"
        "2. IMMEDIATELY run your data tools — do this BEFORE asking the owner anything:\n"
        "   - get_mom_growth_rate() to see if revenue has moved since the plan was created\n"
        "   - get_churn_risk_summary() to see if at-risk customers have changed\n"
        "   - get_repeat_customer_rate() to check retention progress\n"
        "   - Any other tools relevant to the original goal\n"
        "3. Based on what the data shows, ask 1-2 focused questions ONLY about things the data "
        "cannot explain — for example: what the owner tried, what external events happened, "
        "or what got in their way. Never ask about numbers you can already see in the data.\n"
        "4. After the owner responds, produce an UPDATED plan that incorporates both what they "
        "told you AND what the new data shows.\n"
        "5. When you write the updated plan, start it with the exact marker: [[PLAN_UPDATE]]\n\n"
        "TONE: You are a trusted advisor who remembers their situation. Lead with what the numbers "
        "show, then ask only what the data cannot tell you.\n\n"
        "Start now with your opening greeting, then immediately run your data tools."
    )
